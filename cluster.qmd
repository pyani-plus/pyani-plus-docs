...
and this was ample for bacterial genomes.

For larger genomes needing more memory (and longer run-times) we had to
use a different queue, for example the "short" queue allows up to 6 hours
and up to 256GB RAM:
...
possible. Use the logging and debug options within the tool (to keep
the intermediate files and logs).

If you suspect specfic genomes or genome pairs to be a problem, try
reducing the number of genomes to work out which.
...
runs of N). On SLURM the `sacct` command or similar will show when a
job failed with `OUT_OF_MEMORY`. In this case, adjust your Snakemake
`config.yaml` and retry (see example above).
