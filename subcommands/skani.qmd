# `skani` {#skani-subcommand .unnumbered}
...
```bash
pyani-plus skani [OPTIONS] FASTA 
```
:::
...
`--nide`:  Selects one of the `skani` presets. [<span style="color:grey;">Default: `fast`</span>] (<span style="color:green;">choice: `fast`|`medium`|`slow`|`small-genomes` </span>)
