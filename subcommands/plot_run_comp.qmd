# `plot-run-comp` {#plot-run-comp-subcommand .unnumbered}
...
```bash
pyani-plus plot-run-comp [OPTIONS]
```
:::
...
`--run-ids`: Which runs (comma separated list, reference first)? Ex. 1,2,3,4 (reference = 1) (<span style="color:green;">TEXT</span>) [<span style="color:red;">REQUIRED</span>]
